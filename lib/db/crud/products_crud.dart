...
mixin ProductsCrud on CommonCrud {
  double _stockTaxFromMetadata(Map<String, dynamic> metadata) {
    final enabled =
        metadata['tax_enabled'] == 1 || metadata['tax_enabled'] == true;
    return enabled ? ((metadata['tax_rate'] as num?)?.toDouble() ?? 0.0) : 0.0;
  }

  // Products
  Future<List<Map<String, dynamic>>> getProducts(
      {dynamic categoryId, bool includeInactive = false}) async {
    ...
    await db.update('stocks', {'user_id': baseArgs[1]},
        where: 'business_id = ? AND user_id IS NULL', whereArgs: [baseArgs[0]]);
    ...
    if (!stockCols.any((c) => c['name'] == 'discount_limit_type')) {
      await db.execute(
          "ALTER TABLE stocks ADD COLUMN discount_limit_type TEXT DEFAULT 'percentage'");
    }
    if (!stockCols.any((c) => c['name'] == 'discount_limit')) {
      await db.execute(
          "ALTER TABLE stocks ADD COLUMN discount_limit REAL DEFAULT 0");
    }

    final prodCols = await db.rawQuery('PRAGMA table_info(products)');
    if (!prodCols.any((c) => c['name'] == 'discount_limit_type')) {
      await db.execute(
          "ALTER TABLE products ADD COLUMN discount_limit_type TEXT DEFAULT 'percentage'");
    }
  ...
  Future<List<Map<String, dynamic>>> getStocksForProduct(
  ...
  Future<List<Map<String, dynamic>>> searchProducts(String query) async {
    ...
    final branchClause =
        getBranchFilter().replaceAll('branch_id', 'p.branch_id');
  ...
  Future<void> insertProduct(Map<String, dynamic> product) async {
    ...
    await db.update('stocks', {'user_id': uid},
        where: 'business_id = ? AND user_id IS NULL', whereArgs: [bid]);
    ...
    await db.transaction((txn) async {
      ...
      if (isEdit) {
        ...
        final bool metadataChanged = existing == null ||
            ...
            existing['category_id']?.toString() !=
                metadata['category_id']?.toString() ||
            existing['sub_category_id']?.toString() !=
                metadata['sub_category_id']?.toString() ||
            existing['brand_id']?.toString() !=
                metadata['brand_id']?.toString() ||
            existing['unit_id']?.toString() !=
                metadata['unit_id']?.toString() ||
            existing['description']?.toString() !=
                metadata['description']?.toString() ||
            existing['status']?.toString() != metadata['status']?.toString() ||
            existing['is_favorite']?.toString() !=
                metadata['is_favorite']?.toString() ||
            existing['stock_limit']?.toString() !=
                metadata['stock_limit']?.toString() ||
            existing['discount_limit']?.toString() !=
                metadata['discount_limit']?.toString() ||
            existing['discount_limit_type']?.toString() !=
                metadata['discount_limit_type']?.toString() ||
            existing['tax_enabled']?.toString() !=
                metadata['tax_enabled']?.toString() ||
            existing['tax_rate']?.toString() !=
                metadata['tax_rate']?.toString();
      ...
      final discountLimit =
          (product['discount_limit'] as num?)?.toDouble() ?? 0.0;
      final discountLimitType =
          product['discount_limit_type']?.toString() ?? 'percentage';
      ...
      final latestStock =
          latestStockResults.isNotEmpty ? latestStockResults.first : null;
      ...
      if (matchingStocks.isNotEmpty) {
        ...
        await txn.update(
          'stocks',
          {
            ...
            'manufacture_date':
                manufactureDate ?? matchingStocks.first['manufacture_date'],
            'quantity': newQty,
            ...
            'tax': stockTax,
            'pieces_per_pack':
                piecesPerPack ?? matchingStocks.first['pieces_per_pack'],
        ...
        priceChanged =
            isEdit; // only meaningful on edit; new products always create a row
      ...
      await txn.insert('stock_audits', {
        ...
        'new_quantity': newQty,
        'old_purchase_price':
            (latestStock?['cost_price'] as num?)?.toDouble() ?? 0.0,
  ...
  Future<void> toggleProductFavorite(
      dynamic productId, bool currentStatus) async {
    final db = await database;
    await db.update(
      'products',
