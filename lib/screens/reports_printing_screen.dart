...
class _ReportsPrintingScreenState extends State<ReportsPrintingScreen> {
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (ctx, index) {
                        ...
                        final isSelected = selectedOption == option ||
                            (selectedOption == null && isAllOption);

                        return Material(
                          color: Colors.transparent,
                          child: InkWell(
                            onTap: () =>
                                setLocalState(() => selectedOption = option),
                  ...
                  children: [
                    ...
                    _buildSearchableSelector(
                      ...
                      onTap: () async {
                        ...
                        if (result != null)
                          setLocalState(() => selectedCategory = result);
                      },
                    ),
                    ...
                    _buildSearchableSelector(
                      ...
                      onTap: () async {
                        ...
                        if (result != null)
                          setLocalState(() => selectedEmployee = result);
                      },
                    ),
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    selectedValue,
                    style: TextStyle(
                      color: selectedValue.contains('All')
                          ? theme.textSecondary
                          : theme.textPrimary,
                      fontWeight: selectedValue.contains('All')
                          ? FontWeight.normal
                          : FontWeight.w600,
                      fontSize: 14,
                    ),
  ...
  Future<void> _handlePrintCategoryWise() async {
    ...
    try {
      final salesSummary =
          await DatabaseHelper.instance.getCategorySalesSummary(
        categoryId: selectedCategory?['id'],
        startTime: start,
        endTime: end,
      );
      final returnsSummary =
          await DatabaseHelper.instance.getCategoryReturnsSummary(
        categoryId: selectedCategory?['id'],
        startTime: start,
        endTime: end,
      );

      final title = selectedCategory?['id'] == null
          ? 'Overall Category Sales Report'
          : 'Category Report: ${selectedCategory?['name']}';
  ...
  Future<void> _handlePrintGrandSummary() async {
    ...
    try {
      final categorySales = await DatabaseHelper.instance
          .getCategorySalesSummary(startTime: start, endTime: end);
      final categoryReturns = await DatabaseHelper.instance
          .getCategoryReturnsSummary(startTime: start, endTime: end);
      final employeeSales = await DatabaseHelper.instance
          .getEmployeeSalesSummary(startTime: start, endTime: end);
      final paymentSummary = await DatabaseHelper.instance
          .getPaymentMethodSummary(startTime: start, endTime: end);
      final expenses = await DatabaseHelper.instance
          .getExpenses(startTime: start, endTime: end);

      final title = 'Grand Summary Report';
      final subtitle =
          'Period: ${DateFormat('dd MMM yyyy').format(result['startDate'])} to ${DateFormat('dd MMM yyyy').format(result['endDate'])}';
  ...
  Future<void> _handlePrintGeneralSales() async {
    ...
    try {
      ...
      final paymentSummary =
          await DatabaseHelper.instance.getPaymentMethodSummary(
        userId: selectedEmployee?['id'],
        startTime: start,
        endTime: end,
      );
      ...
      final totalPeriodExpenses = expenses.fold<double>(
      ...
      final pdf = await _generateSalesPdf(items, title,
          subtitle: filterInfo,
          returnItems: returns,
  ...
  Future<void> _handlePrintProductWise() async {
    ...
    try {
      ...
      final pdf = await _generateSalesPdf(items, title,
          returnItems: returns, periodExpenses: totalPeriodExpenses);
      _showPreview(pdf, title);
    } finally {
      setState(() => _generatingReportTitle = null);
    }
  ...
  Future<void> _handlePrintStockReport({bool onlyLow = false}) async {
    ...
    try {
      ...
      if (onlyLow) {
        filtered = augmentedProducts
            .where((p) => (p['total_stock'] as num) <= (p['stock_limit'] ?? 0))
            .toList();
      }

      final title =
          onlyLow ? 'Low Stock Alert Report' : 'Current Inventory Status';
  ...
  Future<pw.TextStyle> _getStyle(
      {bool bold = false, double fontSize = 10, PdfColor? color}) async {
              ...
              children: [
                ...
                pw.Column(
                  ...
                  children: [
                    ...
                    pw.Text('GENERATED BY: ${userId.toUpperCase()}',
                        style: pw.TextStyle(
  ...
  Future<pw.Document> _generateSalesPdf(
      ...
      String? subtitle,
      List<Map<String, dynamic>>? paymentMethodSummary,
      double? periodExpenses}) async {
    await _ensureFontsLoaded();
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        header: (context) => _buildReportHeader(
            context, title, business, font, boldFont,
            subtitle: subtitle),
        ...
        build: (context) {
          ...
          // Process Sales Data
          final tableData = List<Map<String, dynamic>>.from(items)
              .asMap()
              .entries
              .map((entry) {
          ...
          if (returnItems != null && returnItems.isNotEmpty) {
            returnTableData = List<Map<String, dynamic>>.from(returnItems)
                .asMap()
                .entries
                .map((entry) {
              final index = entry.key;
            ...
            totalReturnsProfit = totalReturnsProfit.abs();
          ...
          // 1. Sales Details
          widgets.add(pw.Text('SALES DETAILS',
              style: pw.TextStyle(
                  font: boldFont,
                  fontWeight: pw.FontWeight.bold,
                  fontSize: 12)));
          ...
          widgets.add(pw.TableHelper.fromTextArray(
            ...
            border: pw.TableBorder.all(
                width: 1, color: PdfColor.fromHex('#E2E8F0')),
            ...
            cellStyle: pw.TextStyle(
                font: font, fontSize: 8, color: PdfColor.fromHex('#334155')),
          ...
          if (returnTableData.isNotEmpty) {
            ...
            widgets.add(pw.Text('RETURNS DETAILS',
                style: pw.TextStyle(
                    font: boldFont,
                    fontWeight: pw.FontWeight.bold,
                    fontSize: 12)));
          ...
          if (paymentMethodSummary != null) {
            for (var row in paymentMethodSummary) {
              final method =
                  (row['payment_method']?.toString() ?? '').toLowerCase();
          ...
          final summaryContent = pw.Column(
              ...
              children: [
                ...
                pw.SizedBox(height: 12),
                summaryRow('Total Sales (Gross)',
                    '${business.currency} ${totalSalesGross.toStringAsFixed(2)}'),
                summaryRow('Total Sales Discount',
                    '${business.currency} ${totalSalesDiscount.toStringAsFixed(2)}'),
                summaryRow('Net Sales Amount',
                    '${business.currency} ${totalSalesNet.toStringAsFixed(2)}',
                    bold: true),
                if (retNet > 0) ...[
                  pw.SizedBox(height: 4),
                  summaryRow('Total Returns (Net)',
                      '${business.currency} ${retNet.toStringAsFixed(2)}',
                      color: PdfColors.red700),
                ],
                if (expensesToSubtract > 0) ...[
                  pw.SizedBox(height: 4),
                  summaryRow('Total Expenses',
                      '${business.currency} ${expensesToSubtract.toStringAsFixed(2)}',
                      color: PdfColors.red700),
                ],
                ...
                pw.Divider(
                    ...
                    thickness: 0.5),
                pw.SizedBox(height: 10),
                summaryRow('Cash Received',
                    '${business.currency} ${cashAmount.toStringAsFixed(2)}'),
                summaryRow('Credit Amount',
                    '${business.currency} ${creditAmount.toStringAsFixed(2)}'),
                summaryRow('Mobile Transfer',
                    '${business.currency} ${mobileAmount.toStringAsFixed(2)}'),
                summaryRow('Card Payments',
                    '${business.currency} ${cardAmount.toStringAsFixed(2)}'),
                pw.SizedBox(height: 10),
                pw.Divider(
                    borderStyle: pw.BorderStyle.dashed,
                    color: PdfColor.fromHex('#94A3B8'),
                    thickness: 0.5),
                pw.SizedBox(height: 10),
                summaryRow('Final Business Profit',
                    '${finalProfit < 0 ? '-' : ''}${business.currency} ${finalProfit.abs().toStringAsFixed(2)}',
                    bold: true,
                    color: finalProfit < 0
                        ? PdfColors.red700
                        : PdfColors.green700),
  ...
  Future<pw.Document> _generateSummaryPdf(
      String title, List<Map<String, dynamic>> sections,
                  ...
                  : (() {
                      ...
                      for (int i = 0; i < keys.length; i++) {
                        ...
                        if (numericKeys.contains(key)) {
                          ...
                          if (currencyKeys.contains(key)) {
                            totals[i] =
                                '${business.currency} ${sum.toStringAsFixed(2)}';
                          } else {
                            totals[i] = (sum == sum.toInt())
                                ? sum.toInt().toString()
  ...
  Future<pw.Document> _generateStockPdf(
      List<Map<String, dynamic>> products, String title) async {
    await _ensureFontsLoaded();
    final font = _cachedFont!;
    final boldFont = _cachedBoldFont!;
    final pdf = pw.Document();
    ...
    pdf.addPage(
      pw.MultiPage(
        pageFormat: PdfPageFormat.a4,
        header: (context) =>
            _buildReportHeader(context, title, business, font, boldFont),
        footer: (context) => _buildReportFooter(context, font),
        build: (context) => [
          pw.TableHelper.fromTextArray(
            headers: [
  ...
  Future<pw.Document> _generateExpensePdf(
      List<Map<String, dynamic>> expenses, String title) async {
    await _ensureFontsLoaded();
    final font = _cachedFont!;
    final boldFont = _cachedBoldFont!;
    final pdf = pw.Document();
                ...
                children: [
                  pw.Text('Grand Total:',
                      style: pw.TextStyle(
                          ...
                          fontSize: 10,
                          fontWeight: pw.FontWeight.bold)),
                  pw.Text('${business.currency} ${total.toStringAsFixed(2)}',
                      style: pw.TextStyle(
                          font: boldFont,
                          fontSize: 10,
                          fontWeight: pw.FontWeight.bold)),
