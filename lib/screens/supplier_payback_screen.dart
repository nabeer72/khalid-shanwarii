...
class _SupplierPaybackScreenState extends State<SupplierPaybackScreen> {
  ...
  Future<void> _loadPaybackHistory() async {
    try {
      final data =
          await DatabaseHelper.instance.getSupplierPaybacksWithSupplier();
      if (mounted) {
  ...
  Future<void> _showPaybackForm(
      {Supplier? supplier, Map<String, dynamic>? editingPayback}) async {
    final bool isEdit = editingPayback != null;
    final formKey = GlobalKey<FormState>();
    final amountCtrl = TextEditingController(
        text: isEdit ? editingPayback['amount'].toString() : '');
    final noteCtrl = TextEditingController(
        text: isEdit ? editingPayback['notes'] ?? '' : '');
          ...
          if (isLoadingData) {
            ...
            Future.microtask(() async {
              try {
                ...
                int? effectiveSupplierId = isEdit
                    ? editingPayback['supplier_id']
                    : selectedSupplier?.id;
                if (effectiveSupplierId != null) {
                  balance = await DatabaseHelper.instance
                      .getSupplierCreditBalance(effectiveSupplierId);
                  if (isEdit)
                    balance += (editingPayback['amount'] as num).toDouble();
                }
                ...
                if (isEdit && selectedSupplier == null) {
                  try {
                    selectedSupplier = _suppliersWithCredit
                        .firstWhere((s) => s.id == effectiveSupplierId);
                  } catch (_) {
          ...
          return AlertDialog(
                    ...
                    children: [
                        ...
                        _SearchableSupplierDropdown(
                          ...
                          onSelected: (s) async {
                            final b = await DatabaseHelper.instance
                                .getSupplierCreditBalance(s.id ?? 0);
                            setDialogState(() {
                              selectedSupplier = s;
                              balance = b;
                            });
                      ...
                      TextFormField(
                        ...
                        validator: (v) {
                          ...
                          if (val > currentBalance + 0.01)
                            return 'Cannot exceed debt';
                          return null;
                        },
                      ),
                      const SizedBox(height: 16),
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: (isSaving || selectedSupplier == null)
                    ? null
                    : () async {
                        ...
                        try {
                          ...
                          if (isEdit) {
                            await DatabaseHelper.instance.updateSupplierPayback(
                                editingPayback['id'], payback);
                          } else {
                            payback['created_at'] =
                ...
                child: isSaving
                    ...
                    : Text(isEdit ? 'UPDATE PAYBACK' : 'RECORD PAYBACK',
                            ...
                            itemBuilder: (ctx, i) {
                              if (_isHistoryView) {
                                return _PaybackHistoryCard(
                                  ...
                                  onEdit: () => _showPaybackForm(
                                      editingPayback: _filteredItems[i]),
                                );
                              } else {
                                ...
                                return _SupplierCreditCard(
                                  supplier: supplier,
                                  onTap: () =>
                                      _showPaybackForm(supplier: supplier),
                                );
                              }
                            },
                          ),
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            _searchQuery.isEmpty
                ? (_isHistoryView
                    ? 'Records of supplier paybacks will appear here'
                    : 'All suppliers have been cleared')
        ...
        child: ListTile(
          ...
          subtitle: Column(
            ...
            children: [
              ...
              if (payback['notes'] != null &&
                  payback['notes'].toString().isNotEmpty) ...[
                const SizedBox(height: 4),
                Text(payback['notes'],
                    style: TextStyle(
                        color: theme.textHint,
...
class _SearchableSupplierDropdown extends StatefulWidget {
  ...
  const _SearchableSupplierDropdown(
      {required this.suppliers, required this.onSelected});
    ...
    return Column(
      children: [
        TextFormField(
          ...
          decoration: theme
              .glassInputDecoration(
                  'Search Supplier', Icons.business_center_rounded)
              .copyWith(
                isDense: true,
          ...
          Container(
            ...
            child: filtered.isEmpty
                ? const Padding(
                    ...
                    child: Text('No suppliers found with debt',
                      ...
                      return ListTile(
                        ...
                        title: Text(s.name,
                            style: TextStyle(
                                ...
                                fontSize: 12)),
                        subtitle: Text(
                            'Debt: ${BusinessConfig.instance.currencyDisplay} ${s.creditBalance.toStringAsFixed(2)}',
